...
var current_structure_index:int = 0
...
func _ready() -> void:
	...
	road_builder.builder = self
	
	road_placer = RoadPlacer.new()
	road_placer.name = "RoadPlacer"
	add_child(road_placer)
	road_placer.builder = self
	...
	current_structure_id = structures[current_structure_index]
	update_preview()
...
func _process(delta: float) -> void:
	...
	# NOVA LINHA: Se estiver no meio do desenho, atualiza a prévia
	if road_placer.is_placing:
		var current_cell = custom_gridmap.local_to_map(grid_target_position)
		current_cell.y = 0
		road_placer.update_preview(current_cell)
...
func _unhandled_input(event: InputEvent) -> void:
	...
	action_demolish(event, current_cell)

	# Clique Direito + SHIFT: Rotacionar
	action_rotate(event)
	
	# Tecla I: Próxima Estrutura
	action_next_structure(event)
	
	
	# Tecla U: Estrutura Anterior
	action_previous_structure(event)
...
func _load_structures_from_library(type = null) -> Array:
	structures.clear()
	return structure_library.info.keys()
...
func action_build(event: InputEvent) -> void:
	if structures.size() == 0 or not custom_gridmap: return
	var current_build = structure_library.get_info(structures[current_structure_index])
	
	var current_cell = custom_gridmap.local_to_map(grid_target_position)
	current_cell.y = 0
	
	# Verifica se a estrutura atual é do tipo ESTRADA
	var is_road = (current_build.structure_type == StructureData.type.ROAD)
	
	# Evento de apertar o botão (Início)
	if event.is_action_pressed("build"):
		if is_road:
			road_placer.start_placement(current_cell)
			# Esconde a prévia individual padrão do Builder para não bugar o visual
			preview_container.visible = false 
		else:
			# Lógica antiga para prédios normais (Clique Único)
			if map.cash >= current_build.price:
				var orientation = _get_orthogonal_index()
				var success = custom_gridmap.set_composite_structure(
					current_cell, current_build.model_scene, current_build.size,
					orientation, current_build.structure_id
				)
				if success:
	...
	# Evento de SOLTAR o botão do mouse (Confirmação da Estrada)
	elif event.is_action_released("build") and road_placer.is_placing:
		road_placer.confirm_placement()
		preview_container.visible = true # Devolve a prévia normal
		
	# Tecla ESC
	elif event.is_action_pressed("cancel_road") and road_placer.is_placing:
		road_placer.cancel_placement()
		preview_container.visible = true




func action_demolish(_event, _current_cell) -> void:
	# Click Direito: Destruir
	var event = _event
	...
	if event.is_action_pressed("demolish"): # Substitua por sua action (ex: "right_click")
		...
		road_builder.road_fix_neighbors(current_cell)


func action_next_structure(_event) -> void:
	# Tecla I: Próxima Estrutura
	var event = _event
	if event.is_action_pressed("structure_next"):
		current_structure_index = wrap(current_structure_index + 1, 0, structures.size())
		update_preview()


func action_previous_structure(_event) -> void:
	# Tecla U: Estrutura Anterior
	var event = _event
	if event.is_action_pressed("structure_previous"):
		current_structure_index = wrap(current_structure_index - 1, 0, structures.size())
...
func action_rotate(_event) -> void:
	# Clique Direito + SHIFT: Rotacionar
	var event = _event
...
func action_load():
	if Input.is_action_just_pressed("load"):
		...
		for data in map.structures:
			var build_info = structure_library.get_info(data.structure_id)
...
func get_structure_id_from_index(index: int) -> int:
